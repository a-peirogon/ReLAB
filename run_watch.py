...
def parse_args():
    ...
    p.add_argument("--checkpoint", type=str,  default=None,
                   help="Ruta al checkpoint (.pt para DQN, .pkl para Q-Learning)")
    ...
    p.add_argument("--episodes",   type=int,   default=0,
                   help="0 = solo demo, N = entrenar N episodios con visualización")
    p.add_argument("--seed",       type=int,   default=42)
    p.add_argument("--explore",    action="store_true",
                   help="Usar exploración epsilon (solo demo)")
    return p.parse_args()
...
def get_dqn_activations(agent: DQNAgent, obs: np.ndarray) -> list[np.ndarray]:
    """Extrae activaciones reales de cada capa del MLP del DQN."""
    import torch
    agent.policy_net.eval()
    activations = []
    x = torch.FloatTensor(obs).unsqueeze(0).to(agent.device)
    activations.append(obs.copy())  # capa de entrada
...
def get_qlearning_activations(agent: QLearningAgent, obs: np.ndarray) -> list[np.ndarray]:
    """Para Q-Learning tabular, simula activaciones básicas."""
...
def main():
    ...
    episode    = 0
    max_ep     = args.episodes if train_mode else 10_000
