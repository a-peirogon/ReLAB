...
class PygameRenderer:
    ...
    def _compute_visible_layers(self) -> list[int]:
        ...
        return [
            size if (li == 0 or li == n - 1) else min(size, self.MAX_VISIBLE)
            for li, size in enumerate(self.nn_layers)
        ]
    ...
    def _draw_nn_panel(self, obs: np.ndarray, activations: Optional[list[np.ndarray]]) -> None:
        ...
        for li, layer_pos in enumerate(self._node_positions):
            ...
            is_output  = (li == n_out)
            max_a      = float(np.max(np.abs(layer_acts))) + 1e-8
            ...
            for ni, (x, y) in enumerate(layer_pos):
                ...
                if is_output and ni < len(self._output_labels):
                    ...
                    s.blit(self.font_sm.render(f"{prefix}{self._output_labels[ni]}", True, col),
                           (x + radius + 4, y - 6))

        # Leyenda nodos ocultos
    ...
    def _draw_board(
        ...
        pygame.draw.rect(s, GREEN_MID,
                         pygame.Rect(ox-1, oy-1, self.grid_size*cs+2, self.grid_size*cs+2), 1)
        ...
        info = self.font_sm.render(
            f"ep:{episode}  step:{step_count}  ε:{epsilon:.3f}", True, GREEN_DIM)
