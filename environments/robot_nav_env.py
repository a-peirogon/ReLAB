...
ROBOT_RADIUS = 0.3
GOAL_RADIUS = 0.4
_ACTIONS = [(dv, dw) for dv in (-1, 0, 1) for dw in (-1, 0, 1)]
...
class RobotNavEnv(gym.Env):
    ...
    def __init__(self, world_size: float=12.0, n_obstacles: int=N_OBSTACLES_DEFAULT, obstacle_speed: float=0.8, dt: float=0.15, max_steps: int=400, step_penalty: float=-0.01, render_mode: str='none', seed: Optional[int]=None):
        super().__init__()
    ...
    def step(self, action: int) -> tuple[np.ndarray, float, bool, bool, dict]:
        ...
        reward += (self._prev_goal_dist - goal_dist) * 0.5
    ...
    def _get_obs(self) -> np.ndarray:
        vals = [self.v / V_MAX, self.omega / OMEGA_MAX]
        gdist, gbearing = self._relative(self.goal)
        vals += [min(gdist / self._world_diag, 1.0), math.sin(gbearing), math.cos(gbearing)]
        obstacles_sorted = sorted(self.obstacles, key=lambda o: self._dist((self.x, self.y), (o.x, o.y)))
        ...
        for obst in obstacles_sorted:
            ...
            vals += [min(odist / self._world_diag, 1.0), math.sin(obearing), math.cos(obearing), np.clip(rvx_body / (V_MAX + self.obstacle_speed), -1.0, 1.0), np.clip(rvy_body / (V_MAX + self.obstacle_speed), -1.0, 1.0)]
    ...
    def _hits_wall(self) -> bool:
        return not (ROBOT_RADIUS <= self.x <= self.world_size - ROBOT_RADIUS and ROBOT_RADIUS <= self.y <= self.world_size - ROBOT_RADIUS)
    ...
    def _sample_obstacle(self) -> Obstacle:
        ...
        return Obstacle(x=x, y=y, vx=speed * math.cos(angle), vy=speed * math.sin(angle), radius=float(self._np_rng.uniform(0.25, 0.5)))
    ...
    def _get_info(self) -> dict:
        return {'score': int(self.reached_goal), 'steps': self._step_count, 'collided': self.collided}
