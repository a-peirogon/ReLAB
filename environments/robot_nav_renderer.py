...
class RobotNavRenderer:
    ...
    def __init__(self, world_size: float=12.0, px_per_meter: int=45, fps: int=20):
        if not _HAS_PYGAME:
    ...
    def _to_px(self, x: float, y: float) -> tuple[int, int]:
        ...
        py = self.BOARD_PAD + self.board_px - int(y * self.px_per_meter)
        return (px, py)

    def render(self, robot_pose: tuple[float, float, float], goal: tuple[float, float], obstacles: list, episode: int=0, step_count: int=0, agent_name: str='', score: int=0, candidates: Optional[list[dict]]=None, best_idx: int=0) -> bool:
    ...
    def _draw_border(self) -> None:
        pygame.draw.rect(self.screen, GREEN_MID, pygame.Rect(self.BOARD_PAD - 1, self.BOARD_PAD - 1, self.board_px + 2, self.board_px + 2), 1)
    ...
    def _draw_candidates(self, robot_pose: tuple[float, float, float], candidates: list[dict], best_idx: int) -> None:
        rx, ry, rtheta = robot_pose
        cos_t, sin_t = (math.cos(rtheta), math.sin(rtheta))
        scores = [c['score'] for c in candidates if c['score'] > -100000.0]
    ...
    def _draw_info(self, episode: int, step_count: int, agent_name: str, score: int, robot_pose: tuple[float, float, float]) -> None:
        y0 = self.BOARD_PAD * 2 + self.board_px
        _, _, theta = robot_pose
        lines = [f'agent: {agent_name:<10}  ep: {episode:<5}  step: {step_count:<5}  reached: {score}', f'theta: {math.degrees(theta):>6.1f} deg']
