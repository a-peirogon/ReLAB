...
Architecture:
  - Policy network:  MLP (obs_dim → hidden → hidden → n_actions)
  - Target network:  periodically synced copy of policy net
  - Experience replay buffer: uniform random sampling

Key mechanisms:
  - ε-greedy exploration with linear decay
  - Huber loss (smooth L1) for stable gradients
...
class MLP(nn.Module):
    ...
    Intentionally shallow and replaceable — the architecture
    can later be swapped for CNN/attention without changing DQNAgent.
    """
...
class DQNAgent(BaseAgent):
    ...
    epsilon_start / epsilon_end / epsilon_decay : float
        ε-greedy schedule (multiplicative per episode).
    buffer_capacity : int
        Max transitions stored in replay buffer.
    batch_size : int
        Mini-batch size for gradient updates.
    target_update_freq : int
        Hard target network sync every N gradient steps.
    device : str
        'cpu' or 'cuda'.
    """
    ...
    def __init__(
        ...
        # Internal step counter for target sync
        self._grad_steps = 0
    ...
    # ── Store transition + conditional gradient update ─────────────────────────
