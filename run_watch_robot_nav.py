"""
run_watch_robot_nav.py — Visualización en vivo de Robot Navigation.

El punto central del laboratorio: ver el abanico de trayectorias candidatas
de DWA colapsar hacia la ruta segura, o comparar ese planificador clásico
contra un DQN entrenado, en el mismo entorno.

Uso:
    python run_watch_robot_nav.py                              # DWA (por defecto, sin entrenar)
    python run_watch_robot_nav.py --agent dqn                  # DQN sin entrenar (pesos aleatorios)
    python run_watch_robot_nav.py --agent dqn --checkpoint experiments/robot_nav_dqn_run/checkpoint_final.pt
    python run_watch_robot_nav.py --obstacles 8 --fps 15
    python run_watch_robot_nav.py --seed 7
"""
...
def parse_args():
    ...
    p.add_argument("--checkpoint", type=str, default=None,
                   help="Checkpoint .pt para DQN (opcional; sin él usa pesos aleatorios)")
    ...
    p.add_argument("--episodes",   type=int,   default=0,
                   help="0 = demo infinita, N = N episodios y termina")
...
def build_agent(args, obs_dim: int, n_actions: int):
    if args.agent == "dwa":
        return DWAAgent(
            obs_dim=obs_dim, n_actions=n_actions, n_obstacles=args.obstacles,
            world_size=args.world, obstacle_speed=args.obstacle_speed,
        )
...
def main():
    ...
    episode      = 0
    max_episodes = args.episodes if args.episodes > 0 else 10_000
    total_reached = 0
        ...
        while episode < max_episodes:
            ...
            while running:
                ...
                alive = renderer.render(
                    robot_pose = (env.x, env.y, env.theta),
                    goal        = env.goal,
