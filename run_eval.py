"""
run_eval.py — Evaluate a saved agent checkpoint.

Usage:
    python run_eval.py --checkpoint experiments/dqn_run/checkpoint_final.pt
    python run_eval.py --checkpoint experiments/qlearning_run/checkpoint_final.pkl
    python run_eval.py --checkpoint experiments/dqn_run/checkpoint_final.pt --render
    python run_eval.py --checkpoint experiments/dqn_run/checkpoint_final.pt --episodes 20
"""
...
def parse_args():
    ...
    p.add_argument("--delay",      type=float, default=0.05,
                   help="Delay (s) between render frames")
    return p.parse_args()


def load_agent(checkpoint_path: str, obs_dim: int, n_actions: int):
    """Auto-detect agent type from file extension."""
...
def run_eval(agent, env: SnakeEnv, n_episodes: int, render: bool, delay: float):
    ...
    for ep in range(1, n_episodes + 1):
        ...
        print(f"  ep {ep:>3} | reward {total_reward:>7.2f} | "
              f"score {info['score']:>3} | steps {info['steps']:>4}")
